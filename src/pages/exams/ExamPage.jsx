...
const ExamPage = () => {
  ...
  let formatTime = (seconds) => {
    let minute = Math.floor(seconds / 60)
      .toString()
      .padStart(2, "0"); // padStart(2, '0') giúp hiển thị 09 thay vì 9.
  ...
  const handleSubmitExam = async () => {
    const formData = {
      ...
      answers: questions.map((question) => ({
        ...
        selectedOption: answers[question._id] || (question.type === "multiple" ? [] : "") // nếu chưa chọn, gán rỗng
      })),
    };
  ...
  const handleBack = () => {
    confirmAlert({
      message:
        "Bạn có chắc chắn muốn rời khỏi bài thi? Mọi câu trả lời chưa nộp sẽ bị mất.",
      buttons: [
        {
          label: "Rời khỏi",
          ...
          <Col md={3}>
            <Card className="mb-3 sticky-card">
              <Card.Body>
                ...
                <Button onClick={handleBack} variant="danger" className="me-2">
                  Trở về
                </Button>
                <Button onClick={handleSubmitExam} variant="primary">
                  Nộp bài
                </Button>
              </Card.Body>
              <Card.Body>
                ...
                <ProgressBar
                  now={progressPersent}
                  label={`${Math.round(progressPersent)}%`}
                />
              </Card.Body>
            </Card>
          </Col>
                ...
                <div className="d-flex flex-grap gap-2">
                  {questions.map((question, index) => (
                    <Button
                      key={question._id}
                      variant={
                        answers[question._id] ? "primary" : "outline-secondary"
                      }
